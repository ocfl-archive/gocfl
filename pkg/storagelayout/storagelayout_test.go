package storagelayout

import (
	"fmt"
	"gitlab.switch.ch/ub-unibas/gocfl/v2/pkg/checksum"
	"testing"
)

func TestHashAndIdNTupleStorageLayout(t *testing.T) {
	// https://ocfl.github.io/extensions/0003-hash-and-id-n-tuple-storage-layout.html#encapsulation-directory
	// Example 1
	l, err := NewHashAndIdNTuple(&HashAndIdNTupleConfig{
		Config:          Config{ExtensionName: "0003-hash-and-id-n-tuple-storage-layout"},
		DigestAlgorithm: string(checksum.DigestSHA256),
		TupleSize:       3,
		NumberOfTuples:  3,
	})
	if err != nil {
		t.Errorf("error calling NewHashAndIdNTuple(%s, %v, %v) - %v", checksum.DigestSHA256, 3, 3, err)
		return
	}
	fmt.Printf("\nNewHashAndIdNTuple(%s, %v, %v)\n", checksum.DigestSHA256, 3, 3)
	objectID := "object-01"
	testResult := "3c0/ff4/240/object-01"
	rootPath, err := l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashAndIdNTuple(%s) -> %s\n", objectID, rootPath)

	objectID = "..hor/rib:le-$id"
	testResult = "487/326/d8c/%2e%2ehor%2frib%3ale-%24id"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashAndIdNTuple(%s) -> %s\n", objectID, rootPath)

	// https://ocfl.github.io/extensions/0003-hash-and-id-n-tuple-storage-layout.html#encapsulation-directory
	// Example 2
	l, err = NewHashAndIdNTuple(&HashAndIdNTupleConfig{
		Config:          Config{ExtensionName: "0003-hash-and-id-n-tuple-storage-layout"},
		DigestAlgorithm: string(checksum.DigestMD5),
		TupleSize:       2,
		NumberOfTuples:  15,
	})
	if err != nil {
		t.Errorf("error calling NewHashAndIdNTuple(%s, %v, %v) - %v", checksum.DigestMD5, 2, 15, err)
		return
	}
	fmt.Printf("\nNewHashAndIdNTuple(%s, %v, %v)\n", checksum.DigestMD5, 2, 15)
	objectID = "object-01"
	testResult = "ff/75/53/44/92/48/5e/ab/b3/9f/86/35/67/28/88/object-01"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashAndIdNTuple(%s) -> %s\n", objectID, rootPath)

	objectID = "..hor/rib:le-$id"
	testResult = "08/31/97/66/fb/6c/29/35/dd/17/5b/94/26/77/17/%2e%2ehor%2frib%3ale-%24id"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashAndIdNTuple(%s) -> %s\n", objectID, rootPath)

	// https://ocfl.github.io/extensions/0003-hash-and-id-n-tuple-storage-layout.html#encapsulation-directory
	// Example 3
	l, err = NewHashAndIdNTuple(&HashAndIdNTupleConfig{
		Config:          Config{ExtensionName: "0003-hash-and-id-n-tuple-storage-layout"},
		DigestAlgorithm: string(checksum.DigestSHA256),
		TupleSize:       0,
		NumberOfTuples:  0,
	})
	if err != nil {
		t.Errorf("error calling NewHashAndIdNTuple(%s, %v, %v) - %v", checksum.DigestSHA256, 0, 0, err)
		return
	}
	fmt.Printf("\nNewHashAndIdNTuple(%s, %v, %v)\n", checksum.DigestSHA256, 0, 0)
	objectID = "object-01"
	testResult = "object-01"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashAndIdNTuple(%s) -> %s\n", objectID, rootPath)

	objectID = "..hor/rib:le-$id"
	testResult = "%2e%2ehor%2frib%3ale-%24id"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashAndIdNTuple(%s) -> %s\n", objectID, rootPath)

}

func TestHashedNTupleStorageLayout(t *testing.T) {
	// https://ocfl.github.io/extensions/0004-hashed-n-tuple-storage-layout.html
	// Example 1
	l, err := NewHashedNTuple(&HashedNTupleStorage{
		Config:          Config{ExtensionName: "0004-hashed-n-tuple-storage-layout"},
		DigestAlgorithm: string(checksum.DigestSHA256),
		TupleSize:       3,
		NumberOfTuples:  3,
		ShortObjectRoot: false,
	},
	)
	if err != nil {
		t.Errorf("error calling NewHashedNTuple(%s, %v, %v, %v) - %v", checksum.DigestSHA256, 3, 3, false, err)
		return
	}
	fmt.Printf("\nNewHashedNTuple(%s, %v, %v, %v) - %v\n", checksum.DigestSHA256, 3, 3, false, err)
	objectID := "object-01"
	testResult := "3c0/ff4/240/3c0ff4240c1e116dba14c7627f2319b58aa3d77606d0d90dfc6161608ac987d4"
	rootPath, err := l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashAndIdNTuple(%s) -> %s\n", objectID, rootPath)

	objectID = "..hor/rib:le-$id"
	testResult = "487/326/d8c/487326d8c2a3c0b885e23da1469b4d6671fd4e76978924b4443e9e3c316cda6d"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashAndIdNTuple(%s) -> %s\n", objectID, rootPath)

	// https://ocfl.github.io/extensions/0004-hashed-n-tuple-storage-layout.html
	// Example 2
	l, err = NewHashedNTuple(&HashedNTupleStorage{
		Config:          Config{ExtensionName: "0004-hashed-n-tuple-storage-layout"},
		DigestAlgorithm: string(checksum.DigestMD5),
		TupleSize:       2,
		NumberOfTuples:  15,
		ShortObjectRoot: true,
	})
	if err != nil {
		t.Errorf("error calling NewHashedNTuple(%s, %v, %v, %v) - %v", checksum.DigestMD5, 2, 15, true, err)
		return
	}
	fmt.Printf("\nNewHashedNTuple(%s, %v, %v, %v)\n", checksum.DigestMD5, 2, 15, true)
	objectID = "object-01"
	testResult = "ff/75/53/44/92/48/5e/ab/b3/9f/86/35/67/28/88/4e"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashedNTuple(%s) -> %s\n", objectID, rootPath)

	objectID = "..hor/rib:le-$id"
	testResult = "08/31/97/66/fb/6c/29/35/dd/17/5b/94/26/77/17/e0"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashedNTuple(%s) -> %s\n", objectID, rootPath)

	// https://ocfl.github.io/extensions/0004-hashed-n-tuple-storage-layout.html
	// Example 3
	l, err = NewHashedNTuple(&HashedNTupleStorage{
		Config:          Config{ExtensionName: "0004-hashed-n-tuple-storage-layout"},
		DigestAlgorithm: string(checksum.DigestSHA256),
		TupleSize:       0,
		NumberOfTuples:  0,
		ShortObjectRoot: false,
	},
	)
	if err != nil {
		t.Errorf("error calling NewHashedNTuple(%s, %v, %v, %v) - %v", checksum.DigestSHA256, 0, 0, false, err)
		return
	}
	fmt.Printf("\nNewHashedNTuple(%s, %v, %v, %v)\n", checksum.DigestSHA256, 0, 0, false)
	objectID = "object-01"
	testResult = "3c0ff4240c1e116dba14c7627f2319b58aa3d77606d0d90dfc6161608ac987d4"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashedNTuple(%s) -> %s\n", objectID, rootPath)

	objectID = "..hor/rib:le-$id"
	testResult = "487326d8c2a3c0b885e23da1469b4d6671fd4e76978924b4443e9e3c316cda6d"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("HashedNTuple(%s) -> %s\n", objectID, rootPath)

}

func TestFlatDirectoryStorageLayout(t *testing.T) {
	// https://ocfl.github.io/extensions/0002-flat-direct-storage-layout.html
	// Example 1
	l := FlatDirect{}
	objectID := "object-01"
	testResult := "object-01"
	rootPath, err := l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s", objectID)
	}
	if rootPath != testResult {
		t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
	}
	fmt.Printf("FlatDirect(%s) -> %s\n", objectID, rootPath)

	objectID = "..hor_rib:lé-$id"
	testResult = "..hor_rib:lé-$id"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s - %v", objectID, err)
	} else {
		if rootPath != testResult {
			t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
		} else {
			fmt.Printf("FlatDirect(%s) -> %s\n", objectID, rootPath)
		}
	}

	// https://ocfl.github.io/extensions/0002-flat-direct-storage-layout.html
	// Example 2
	objectID = "info:fedora/object-01"
	testResult = "info:fedora/object-01"
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s - %v", objectID, err)
	} else {
		if rootPath != testResult {
			t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
		} else {
			fmt.Printf("FlatDirect(%s) -> %s\n", objectID, rootPath)
		}
	}

	objectID = ""
	testResult = ""
	rootPath, err = l.ID2Path(objectID)
	if err != nil {
		t.Errorf("cannot convert %s - %v", objectID, err)
	} else {
		if rootPath != testResult {
			t.Errorf("%s -> %s != %s", objectID, rootPath, testResult)
		} else {
			fmt.Printf("FlatDirect(%s) -> %s\n", objectID, rootPath)
		}
	}

}
