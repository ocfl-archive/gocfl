...
type StorageRoot interface {
	...
	CreateExtensions(fsys fs.FS, validation validation.Validation) (ExtensionManagerCore, error)
	Check() error
	...
	Init(ver version.OCFLVersion, digest checksum.DigestAlgorithm, manager ExtensionManagerCore) error
	Load() error
	IsModified() bool
	SetModified()
