...
func (s *stateBase) CopyFrom(state inventory.State) error {
	state2, ok := state.(*stateBase)
	if !ok {
		return errors.WithStack(inventory.StateTypeDifferent)
	}
	s.err = state2.Err()
...
func (s *stateBase) Equals(state inventory.State) bool {
	if s == nil || state == nil {
		return false
	}
