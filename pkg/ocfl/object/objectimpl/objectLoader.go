...
func NewObjectLoader(ctx context.Context, fsys fs.FS, fact factory.Factory, logger ocfllogger.OCFLLogger) *inventoryLoader {
...
func (loader *inventoryLoader) loadInventory(data []byte) (inventory.Inventory, error) {
	...
	inventory := loader.factory.NewInventory(loader.ctx).WithWriteable()
	/*
...
func (loader *inventoryLoader) Load(folder string) (inventory.Inventory, error) {
	// load inventory file
	...
	inventory, err := loader.loadInventory(inventoryBytes)
	if err != nil {
	...
	if err != nil {
		if errors.Is(errors.Cause(err), fs.ErrNotExist) {
			validation.AddValidationErrors(loader.ctx, validation.GetValidationError(inventory.GetOCFLVersion(), validation.E058).AppendDescription("sidecar '%v/%s' does not exist", loader.fsys, sidecarPath))
		} else {
			validation.AddValidationErrors(loader.ctx, validation.GetValidationError(inventory.GetOCFLVersion(), validation.E060).AppendDescription("cannot read sidecar '%v/%s': %v", loader.fsys, sidecarPath, err.Error()))
		}
		...
		if /* matches == nil || */ len(matches) == 0 {
			validation.AddValidationErrors(loader.ctx, validation.GetValidationError(inventory.GetOCFLVersion(), validation.E061).AppendDescription("no suffix \" inventory.json\" in '%v/%s'", loader.fsys, sidecarPath))
		} else {
			...
			if digestString != inventoryDigestString {
				validation.AddValidationErrors(loader.ctx, validation.GetValidationError(inventory.GetOCFLVersion(), validation.E060).AppendContext("'%s' != '%s'", digestString, inventoryDigestString))
			}
		}
	}
	return inventory, inventory.Finalize(false)
