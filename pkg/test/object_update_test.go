...
func TestObjectUpdate(t *testing.T) {
	...
	// Überprüfe ob Inhaltsdateien in v1 existieren
	_, err = vfs.Stat("vfs://testmem/" + objFolder + "/v1/content/" + testFileName1)
	assert.NoError(t, err, "Content file in v1 should exist")

	// Überprüfe ob Inhaltsdateien in v2 existieren
	_, err = vfs.Stat("vfs://testmem/" + objFolder + "/v2/content/" + testFileName2)
	assert.NoError(t, err, "New content file in v2 should exist")

	_, err = vfs.Stat("vfs://testmem/" + objFolder + "/v2/content/" + testFileName1)
	assert.NoError(t, err, "Updated content file in v2 should exist")

	// 8. Objekt laden und Inventory prüfen
