...
func NewFileBuffer(name string, b []byte) *Buffer {
	return &Buffer{name: name, Buff: bytes.NewBuffer(b)}
}
...
// into memory.
func NewFileBufferFromReader(name string, reader io.Reader) (*Buffer, error) {
	data, err := io.ReadAll(reader)
	if err != nil {
		return nil, err
	}
	return NewFileBuffer(name, data), nil
}
...
func (f *Buffer) Stat() (fs.FileInfo, error) {
	return &ObjectFileInfoFile{
		name:    f.name,
		modTime: time.Now(),
		size:    int64(f.Buff.Len()),
	}, nil
}
